...
prova.test("csv.parse is header-aware", { spec = "api-freeze §1 - csv module" }, function(t)
...
prova.test("csv.parse honors quoted fields with embedded commas", { spec = "api-freeze §1 - csv module" }, function(t)
  local rows = csv.parse('name,desc\nredis,"fast, in-memory"\n')
  t:expect(rows[1].desc):equals("fast, in-memory")
end)

prova.test("csv.encode emits headers + rows, round-tripping parse", { spec = "api-freeze §1 - csv module" }, function(t)
