...
| `[topologies.<name>]` | `package` (required) + exactly one of `topology` (the package's advertised name) or `factory` (dotted path), optional `requires` and `options` (passed to the factory), `startup` (how long it needs to come up), and `scope` — `"file"` (default: one instance per declaring proof file) or `"run"` (one instance for the whole run, shared by every declaring file; see docs/design/topologies.md#run-wide-topology-is-provisioned-once). Sugar for `prova.topology(name, require(package).<factory>)` — the name `prova up <name>` and proofs both address. |
