-- yaml.decode (single doc) and yaml.decode_all (multi-doc `---` stream, as in k8s manifests).

prova.test("parses a single document into a table", function(t)
  local doc = yaml.decode("name: widget\nreplicas: 3\nlabels:\n  app: web\n")
...
prova.test("raises on invalid yaml", function(t)
  local ok = pcall(function() yaml.decode("key: [unterminated\n") end)
