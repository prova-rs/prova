...
- **CLAIMED** — `<!-- claim: id -->` in a doc under a `[specs]` source. Prose became an obligation.
...
See also: `prova learn claims` (the owed half) · `prova learn promises` (the open half) · `prova
learn record` (what did NOT run) · `prova learn reminders` (what is due)
